...
struct process_stats{
	struct file *my_exectutable;
	struct file *file[128];
	struct mmaped_files *mfile[128];
	struct children_stats *pos;
	struct semaphore parent_died;
};
...
//#ifdef USERPROG
    ...
    struct hash apt;
    struct lock apt_lock;
    struct list children;
    void *memloc;
    struct process_stats my_stats;
//#endif
    ...
    ////// list of locks held by me ////////
    struct list lock_list;

    //struct list block_list;
    struct lock *waiting_lock;

    // integer pointers for recent_cpu and nice
    int rec_cpu, nice;
  };
