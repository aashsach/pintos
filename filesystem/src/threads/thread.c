...
//#endif
  ...
  while(!list_empty(&t->children)){
	  ...
	  if(!sema_try_down(&temp->child_dead)) temp->t->my_stats.pos=NULL;
	  free(temp);
  }
  /////////
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  t->my_stats.pwd_sector=1;

  //userprogram
  list_init(&t->children);
  if(t!=initial_thread){
	  struct thread *cur=thread_current();
	  t->my_stats.pwd_sector=cur->my_stats.pwd_sector;
