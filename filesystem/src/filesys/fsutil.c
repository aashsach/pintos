...
fsutil_cat (char **argv)
{
  ...
  struct dir *parent=NULL;
    ...
    char *file_name_=path_parser(file_name,path,&parent);
    if(!parent)
  	  PANIC ("%s: open failed", file_name);
  file = filesys_open_nested(file_name_,parent);
  if (file == NULL)
    PANIC ("%s: open failed", file_name);
...
fsutil_rm (char **argv) 
{
  ...
  struct dir *parent=NULL;
  char *path=malloc(strlen(file_name)+1);
  char *file=path_parser(file_name,path,&parent);
  if(!parent)
  	  PANIC ("%s: delete failed", file_name);
  if (!filesys_remove_nested(file,parent))
  ...
  for (;;)
    {
      ...
      if (type == USTAR_EOF)
      ...
      else if (type == USTAR_REGULAR)
        {
          ...
          if (!filesys_create_nested(file,size,parent))
            PANIC ("%s: create failed", file_name);
          dst = filesys_open_nested(file,parent);
          if (dst == NULL)
            PANIC ("%s: open failed", file_name);
...
fsutil_append (char **argv)
{
  ...
  /* Open source file. */
  if(file)
	  src=filesys_open_nested(file,parent);
